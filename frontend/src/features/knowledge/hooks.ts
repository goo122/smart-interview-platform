...
import type { KnowledgeDocumentList } from "@/features/knowledge/types";
...
export function useKnowledgeBaseReadiness() {
  ...
  const documentQueries = useQueries({
    queries: (basesQuery.data?.records ?? []).map((base) => ({
      ...
      staleTime: 1_000,
      refetchInterval: (query: Query<KnowledgeDocumentList, Error>) =>
        (query.state.data?.records ?? []).some(isDocumentProcessing)
          ? 2_000
          : false,
      refetchIntervalInBackground: false,
    })),
...
export function useKnowledgeMutations() {
  ...
  const invalidateBases = () =>
    queryClient.invalidateQueries({ queryKey: knowledgeKeys.bases() });
  ...
  const upload = useMutation({
    ...
    onSuccess: (_, variables) =>
      queryClient.invalidateQueries({
        queryKey: knowledgeKeys.documents(variables.baseId),
      }),
  });
