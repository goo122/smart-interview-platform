...
const createSyntheticPdf = () => {
  const content =
    "BT /F1 12 Tf 72 720 Td (Synthetic backend engineer resume.) Tj ET";
  const objects = [
...
test.describe("real provider smoke", () => {
  ...
  test("runs the explicitly enabled DashScope provider smoke flow", async ({
    page,
  }) => {
    ...
    const base = await postJson<{ id: string }>(
    ...
    await expect
      ...
      .toBe("READY");
    const prepared = await json<JsonRecord>(
      `/api/xunzhi/v1/interview/sessions/${sessionId}`,
    );
    ...
    expect((await preview.body()).subarray(0, 5).toString("ascii")).toBe(
      "%PDF-",
    );
    ...
    expect(
      (questions[0]?.citations as unknown[] | undefined)?.length ?? 0,
    ).toBeGreaterThan(0);
    ...
    expect(
      (await json<JsonRecord>(`/api/xunzhi/v1/interview/sessions/${sessionId}`))
        .status,
    ).toBe("COMPLETED");

    const reportStart = await postJson<JsonRecord>(
    ...
    await expect
      ...
      .toBe("READY");
    const report = await json<JsonRecord>(
      `/api/xunzhi/v1/interview/sessions/${sessionId}/report`,
    );
    ...
    expect(
      (report.radarData as JsonRecord[]).some(
        (item) => item.dimension === "resume",
      ),
    ).toBeTruthy();
    safeCounters.chatInvocations += 1;
