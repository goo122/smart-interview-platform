...
> 文档版本：1.1
> 生成日期：2026-09-01
> 适用项目版本：当前 `master` 分支，源码基线 `6d4596b`（`fix: complete Xunfei TTS task retrieval`）
> 适用范围：项目根目录（`.`）
...
| 语音合成 | 已实现接口和前端播放/需配置 | 支持 Fake WAV 或讯飞长文本语音合成；当前讯飞适配器支持 `x4_mingge`，任务结果是短期内存记录。 |
| 摄像头预览 | 已实现 | 浏览器本地 `getUserMedia` 预览、关闭和截图。 |
| 神态分析 | 已实现/需配置 | 前端按能力探测结果上传摄像头 JPEG/PNG 帧；后端完成鉴权、图片校验、限流、结构化分析结果校验、样本持久化和报告汇总。默认 Provider 为 `unavailable`，真实使用需配置视觉模型。 |
| 历史会话和历史面试 | 已实现 | 普通对话、面试会话、报告均可分页查询。 |

### 1.4 当前完成情况

当前仓库是一个 MVP 发布候选版本，具备可运行的 React/Vite 前端、FastAPI 后端、PostgreSQL/pgvector、Redis、后台 Worker、数据库迁移和自动化测试。讯飞长文本语音合成已完成配置兼容和真实音频返回验证，但仍依赖用户在讯飞控制台开通发音人及字符额度。摄像头神态分析的前后端链路、数据库记录和报告汇总也已实现，但真实分析仍依赖视觉模型 Provider 配置。项目 README 明确支持三类运行模式：
...
- 语音合成单独支持 `unavailable`、`fake`、`xunfei`；讯飞实现使用长文本语音合成 HTTP 接口，创建任务后轮询任务状态并下载音频。
...
- **用途**：将 AI 消息转换为可播放音频。
- **主要流程**：前端查询 TTS 能力 → 调用同步 `synthesize` 或兼容 `tasks` 接口 → 后端校验文本、音频格式、发音人、限流和幂等 → 讯飞长文本接口创建任务 → 使用 `app_id` 和 `task_id` 轮询任务 → 解码并下载讯飞音频 → 后端以 Base64 返回 → 前端创建/复用 Object URL 播放。
...
- **依赖服务**：讯飞长文本语音合成或 Fake Provider；前端浏览器音频播放。
- **已实现兼容处理**：讯飞长文本查询请求只发送 `app_id` 和 `task_id`；返回的 `http` 或 `https` 音频地址仅允许 `*.xf-yun.com` 主机；`lame` 返回结果按 MP3/`audio-mpeg` 交给前端播放。此前错误传递 `sid` 会导致 `10163 unknown field`，已在源码基线 `6d4596b` 修复。
- **限制**：默认不可用；当前讯飞适配器仅声明 `lame` 音频格式和 `x4_mingge` 发音人；截图中“超拟人语音合成”的 `x5_*`/`x7_*` 发音人不属于当前适配器支持范围，需要另行实现 WebSocket 适配器；任务记录只存在进程内存并按 TTL 清理，不是持久化队列；默认文本上限 10000 字符、音频上限 5 MiB、每用户每分钟 30 次；服务重启后短期任务结果不会保留。

### 3.12 摄像头与神态分析

- **摄像头预览已实现**：`CameraPreview.tsx` 使用 `navigator.mediaDevices.getUserMedia` 获取视频流，支持关闭、展开和从视频帧生成 JPEG Blob。面试页默认打开摄像头，可手动关闭。
- **神态分析已实现**：`useInterviewDemeanorPolling.ts` 先调用能力接口，再按照后端返回的最小间隔上传摄像头帧；FastAPI 提供 `GET /api/xunzhi/v1/interview/demeanor/capabilities` 和 `POST /api/xunzhi/v1/interview/sessions/{sessionId}/demeanor-evaluation`。后端会校验面试归属和 `IN_PROGRESS` 状态、JPEG/PNG 格式、文件大小和像素数，并执行本地/Redis 限流和并发冲突保护。
- **分析与存储**：`DemeanorAnalysisService` 调用 Fake 或视觉模型 Provider，将结果校验为 `StructuredDemeanorEvaluation`，保存总体分数、视线稳定性、坐姿、面部可见度、表达自然度、建议和置信度。只保存无原图的结构化样本；报告生成时会将样本平均分加入“仪态表达”雷达维度。
- **当前配置限制**：`APP_DEMEANOR_ANALYSIS_PROVIDER` 默认是 `unavailable`，因此未配置 Provider 时能力接口会返回不可用，前端不会持续上传截图。开发/测试可使用 `fake`；真实环境应使用支持视觉输入和结构化输出的 `openai_compatible` 模型，并配置 `APP_LLM_API_KEY`、`APP_LLM_BASE_URL` 以及 `APP_DEMEANOR_ANALYSIS_MODEL` 或 `APP_LLM_MODEL`。生产环境禁止 Fake Provider。
- **安全边界**：当前实现只允许分析画面中可直接观察的呈现信号，不推断身份、人格、情绪、健康、诚信或心理状态；原始摄像头图片不写入数据库。浏览器摄像头权限仍由用户控制，正式环境应使用 HTTPS 或 localhost，并在产品界面补充清晰的上传授权和数据保留说明。
...
| GET | `/api/xunzhi/v1/interview/demeanor/capabilities` | 是 | 查询神态分析是否可用、Provider、图片大小/像素限制、最小请求间隔和分析版本。 |
| POST | `/api/xunzhi/v1/interview/sessions/{sessionId}/demeanor-evaluation` | 是 | 上传 multipart 字段 `userPhoto` 的 JPEG/PNG 摄像头帧，返回结构化神态分析样本；仅允许进行中的当前用户面试。 |
...
| `APP_DEMEANOR_ANALYSIS_PROVIDER` | `unavailable` | 神态分析 Provider：`unavailable`、`fake`、`openai_compatible`；真实使用需视觉模型。 |
| `APP_DEMEANOR_ANALYSIS_MODEL` | 留空 | 神态分析专用模型；留空时复用 `APP_LLM_MODEL`，需要支持视觉输入和结构化输出。 |
...
神态分析的运行参数还包括 `APP_DEMEANOR_ANALYSIS_MAX_IMAGE_BYTES`（默认 2 MiB）、`APP_DEMEANOR_ANALYSIS_MAX_PIXELS`（默认 8000000）、`APP_DEMEANOR_ANALYSIS_MIN_INTERVAL_SECONDS`（默认 5 秒）和 `APP_DEMEANOR_ANALYSIS_REQUEST_TIMEOUT_SECONDS`（默认 30 秒）。这些变量只接受安全示例值，真实密钥仍只能放在后端环境变量或密钥管理系统中。
...
| `APP_TTS_AUDIO_FORMAT` | `lame` | 当前讯飞长文本适配器使用 `lame`；Provider 实际返回 MP3 兼容音频。 |
| `APP_TTS_SAMPLE_RATE` / `APP_TTS_VOICE` | `16000` / `x4_mingge` | 采样率和默认发音人；当前讯飞适配器只支持 `x4_mingge`。 |
...
讯飞 TTS 的三个专用凭据可以单独配置；如果为空，当前工厂会复用已配置的 `APP_XUNFEI_ASR_APP_ID`、`APP_XUNFEI_ASR_API_KEY` 和 `APP_XUNFEI_ASR_API_SECRET`。这只表示使用同一讯飞应用凭据，不代表自动获得 TTS 产品权限。真实调用仍需在讯飞控制台开通长文本语音合成、授权 `x4_mingge` 并保证字符额度充足。
...
- `git diff --check` 通过。
- TTS 回归测试：新增查询参数和 HTTP 音频地址用例，`2 passed`。
- 讯飞真实 TTS 冒烟测试：任务创建、查询和音频下载成功，返回 MP3 兼容音频；本次测试文本仅用于验证链路，不代表所有账户额度长期充足。
...
| TTS 音频下载 | 讯飞音频下载仅接受 HTTP/HTTPS 且主机名以 `.xf-yun.com` 结尾，避免把任意外部地址当作音频源。 |
| 摄像头与神态分析 | 仅接受 JPEG/PNG，限制文件大小和像素数；校验当前用户和进行中的面试；本地/Redis 限流与并发保护；只保存结构化结果，不保存原始图片；模型提示词限制为可见呈现信号。 |
...
- 为摄像头帧上传和神态分析补充产品层面的明确授权、数据保留期限、删除入口和隐私告知；服务端分析接口和无原图存储链路已经实现。
...
1. 登录后调用 `GET /api/xunzhi/v1/speech/tts/capabilities`；若 `available=false`，检查 `APP_TEXT_TO_SPEECH_PROVIDER` 是否为 `xunfei`，以及 TTS 凭据或可复用的 ASR 凭据是否完整。
2. 当前讯飞长文本接口使用 `APP_TTS_AUDIO_FORMAT=lame` 和 `APP_TTS_VOICE=x4_mingge`。如果使用 `wav` 或未授权发音人，可能收到 `10163` 参数校验错误。
3. 如果收到 `11200`、`licc limit` 或类似授权/额度错误，检查讯飞控制台的长文本语音合成权限、发音人授权和剩余字符数；项目配置正确不等于账户已有额度。
4. 如果收到查询任务时 `sid unknown field` 或 `10163`，确认运行的是包含提交 `6d4596b` 的镜像，并执行：

```powershell
docker compose up -d --build --force-recreate --no-deps api
```

5. 如果创建任务成功但下载失败，检查返回音频地址是否为讯飞 `*.xf-yun.com` 主机，并确认 API 容器可以访问外网；当前适配器支持讯飞返回的 HTTP/HTTPS 音频地址。

### 13.10 神态分析不可用或分析失败

1. 登录后调用 `GET /api/xunzhi/v1/interview/demeanor/capabilities`；如果返回 `available=false`，说明当前 `APP_DEMEANOR_ANALYSIS_PROVIDER` 仍为 `unavailable`，或 Provider 初始化没有完成。修改后需要重启 API 容器。
2. 真实 Provider 需要支持图片输入和结构化输出的视觉模型，并配置 `APP_LLM_API_KEY`、`APP_LLM_BASE_URL` 以及 `APP_DEMEANOR_ANALYSIS_MODEL` 或 `APP_LLM_MODEL`。普通文本模型不一定支持该功能；千问配置也必须选择具备视觉输入能力的模型。
3. 如果返回 400，检查浏览器是否生成了完整的 JPEG/PNG 截图，以及图片大小和像素数是否在能力接口返回的限制内；如果返回 409，等待上一帧分析完成或达到最小请求间隔后再试。
4. 如果返回 503，检查视觉模型服务、网络、凭据、模型权限和请求超时设置。前端能力探测失败时会停止截图轮询，避免在服务不可用时持续上传。
5. 分析成功后后端只保存结构化样本，不保存原始摄像头图片；面试报告只在存在有效样本时加入“仪态表达”雷达维度。
...
- 摄像头预览和神态分析后端链路已实现；默认 Provider 为 `unavailable`，未配置视觉模型时不能执行真实分析。当前分析是按间隔上传的单帧观察，不是连续视频理解；报告只在存在有效样本时加入“仪态表达”维度。
...
- 为神态分析补充真实视觉模型的生产验证、失败重试/可观测指标、用户授权与数据删除界面；这些属于产品和生产化增强，不代表当前接口未实现。
...
- 最新提交 `19d9942`、`6d4596b` 及讯飞长文本 TTS 配置、查询和音频下载修复。
